...
static struct nla_policy sparx5_qos_fp_port_genl_policy[SPARX5_QOS_FP_PORT_ATTR_END] = {
	[SPARX5_QOS_FP_PORT_ATTR_NONE] = { .type = NLA_UNSPEC },
	[SPARX5_QOS_FP_PORT_ATTR_CONF] = { .type = NLA_BINARY,
		.len = sizeof(struct sparx5_qos_fp_port_conf) },
	[SPARX5_QOS_FP_PORT_ATTR_STATUS] = { .type = NLA_BINARY,
		.len = sizeof(struct sparx5_qos_fp_port_status) },
};

static int sparx5_qos_fp_port_genl_conf_set(struct sk_buff *skb,
					    struct genl_info *info)
{
	struct sparx5_qos_fp_port_conf nl_conf;
	...
	if (!info->attrs[SPARX5_QOS_FP_PORT_ATTR_IDX]) {
		pr_err("ATTR_IDX is missing\n");
		return -EINVAL;
	}

	ifindex = nla_get_u32(info->attrs[SPARX5_QOS_FP_PORT_ATTR_IDX]);

	dev = __dev_get_by_index(net, ifindex);
	if (!sparx5_netdevice_check(dev))
		return -EOPNOTSUPP;

	if (!info->attrs[SPARX5_QOS_FP_PORT_ATTR_CONF]) {
		pr_err("SPARX5_QOS_PORT_ATTR_CONF is missing\n");
		return -EINVAL;
	}

	nla_memcpy(&nl_conf, info->attrs[SPARX5_QOS_FP_PORT_ATTR_CONF],
		   nla_len(info->attrs[SPARX5_QOS_FP_PORT_ATTR_CONF]));
					    ...
					    struct genl_info *info)
{
	...
	if (nla_put(msg, SPARX5_QOS_FP_PORT_ATTR_CONF, sizeof(nl_conf),
		    &nl_conf)) {
...
static int sparx5_qos_fp_port_genl_status_get(struct sk_buff *skb,
					      struct genl_info *info)
{
	struct sparx5_qos_fp_port_status nl_status = {};
	...
	if (nla_put(msg, SPARX5_QOS_FP_PORT_ATTR_STATUS, sizeof(nl_status),
		    &nl_status)) {
		pr_err("Failed nla_put\n");
		err = -EMSGSIZE;
		goto nla_put_failure;
...
static struct genl_ops sparx5_qos_fp_port_genl_ops[] = {
	{
		.cmd    = SPARX5_QOS_FP_PORT_GENL_CONF_SET,
		.doit   = sparx5_qos_fp_port_genl_conf_set,
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.flags  = GENL_ADMIN_PERM,
	},
	{
		.cmd    = SPARX5_QOS_FP_PORT_GENL_CONF_GET,
		.doit   = sparx5_qos_fp_port_genl_conf_get,
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.flags  = GENL_ADMIN_PERM,
	},
	{
		.cmd    = SPARX5_QOS_FP_PORT_GENL_STATUS_GET,
...
static struct genl_family sparx5_qos_fp_port_genl_family = {
	/* Reuse Maserati name until further. LAN966x is used by MCHP QoS tool*/
	.name		= "lan966x_netlink",
	.hdrsize	= 0,
	.version	= 1,
	.maxattr	= SPARX5_QOS_FP_PORT_ATTR_MAX,
