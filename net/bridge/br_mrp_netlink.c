...
br_mrp_instance_policy[IFLA_BRIDGE_MRP_INSTANCE_MAX + 1] = {
	...
	[IFLA_BRIDGE_MRP_INSTANCE_VID]		= { .type = NLA_U16 },
};
				 ...
				 int cmd, struct netlink_ext_ack *extack)
{
	...
	if (tb[IFLA_BRIDGE_MRP_INSTANCE_VID])
		inst.vid = nla_get_u16(tb[IFLA_BRIDGE_MRP_INSTANCE_VID]);
...
int br_mrp_fill_info(struct sk_buff *skb, struct net_bridge *br)
{
	...
	hlist_for_each_entry_rcu(mrp, &br->mrp_list, list) {
		...
		if (nla_put_u16(skb, IFLA_BRIDGE_MRP_INFO_PRIO,
				mrp->prio))
			goto nla_put_failure;
		if (nla_put_u16(skb, IFLA_BRIDGE_MRP_INFO_VID,
				mrp->vid))
			goto nla_put_failure;
